...
INSERT INTO custom_pages_i18n (id, custom_page_id, locale, title, slug, content, summary, meta_title, meta_description) VALUES
  ('cpi-kvkk-tr', 'cp-kvkk', 'tr', 'KVKK Aydınlatma Metni', 'kvkk',
   '{"html":"<h2>KVKK Aydınlatma Metni</h2><p><strong>Veri Sorumlusu:</strong> Orhan Güzel – Softwareentwicklung (Gold Mood Astro markası) · Stralsunder Str. 38, 41515 Grevenbroich, Deutschland</p><h3>İşlenen Veriler</h3><ul><li>Hesap bilgileri: ad, e-posta, telefon</li><li>Doğum verisi: tarih, saat, şehir (yorum üretimi için)</li><li>Ödeme: Stripe veya PayPal tarafından işlenir; karta dair veriyi saklamayız</li><li>Kullanım: oturum logları, IP, cihaz bilgisi (güvenlik)</li></ul><h3>Amaçlar</h3><p>Hizmet sunumu, ödeme/randevu işleme, kişisel astroloji yorumu üretimi, danışan desteği, yasal yükümlülükler.</p><h3>Haklarınız</h3><p>KVKK md.11 kapsamında; verilerinize erişme, düzeltme, silme, sınırlama ve veri taşınabilirliği haklarınız vardır. Hesap silme talebi 7 gün soğuma süresiyle işlenir.</p><h3>İletişim</h3><p>goldmoodastro@gmail.com</p>"}',
  ...
  ('cpi-kvkk-en', 'cp-kvkk', 'en', 'GDPR Notice', 'gdpr',
   '{"html":"<h2>GDPR / Data Processing Notice</h2><p><strong>Data Controller:</strong> Orhan Güzel – Softwareentwicklung (Gold Mood Astro brand) · Stralsunder Str. 38, 41515 Grevenbroich, Germany</p><h3>Data we process</h3><ul><li>Account: name, email, phone</li><li>Birth data: date, time, city (for personal readings)</li><li>Payment: handled by Stripe or PayPal; no card data stored</li><li>Usage: session logs, IP, device (security)</li></ul><h3>Purposes</h3><p>Service delivery, payment & booking, personal astrology readings, customer support, legal obligations.</p><h3>Your rights</h3><p>Access, rectification, deletion, restriction, portability. Account deletion requests have a 7-day cooling-off period.</p><h3>Contact</h3><p>support@goldmoodastro.com</p>"}',
  ...
  ('cpi-kvkk-de', 'cp-kvkk', 'de', 'DSGVO-Hinweis', 'dsgvo',
   '{"html":"<h2>DSGVO-Hinweis</h2><p><strong>Verantwortlicher:</strong> Orhan Güzel – Softwareentwicklung (Marke Gold Mood Astro) · Stralsunder Str. 38, 41515 Grevenbroich, Deutschland</p><h3>Verarbeitete Daten</h3><ul><li>Konto: Name, E-Mail, Telefon</li><li>Geburtsdaten: Datum, Uhrzeit, Ort</li><li>Zahlung: über Stripe oder PayPal; keine Kartendaten gespeichert</li><li>Nutzung: Sitzungsprotokolle, IP, Gerät</li></ul><h3>Ihre Rechte</h3><p>Auskunft, Berichtigung, Löschung, Einschränkung, Übertragbarkeit. 7 Tage Bedenkzeit bei Löschanträgen.</p>"}',
...
INSERT INTO custom_pages_i18n (id, custom_page_id, locale, title, slug, content, summary, meta_title, meta_description) VALUES
  ...
  ('cpi-priv-en', 'cp-gizlilik', 'en', 'Privacy Policy', 'privacy',
   '{"html":"<h2>Privacy Policy</h2><p>Your privacy is a priority. This policy explains what we collect, why, and how we protect it.</p><h3>What we collect</h3><p>Account info, birth data, usage metrics, payment tokens (not card numbers).</p><h3>Retention</h3><p>Active account lifetime + legal retention. After deletion, data is irreversibly removed within 7 days.</p><h3>Third parties</h3><p>Stripe/PayPal (payments), Cloudinary (image CDN), LiveKit (audio/video), Anthropic/OpenAI (reading generation).</p>"}',
