  ...
  const cards = args.cards.map((card) => {
    ...
    const topicBridge: Record<QuestionTopic, string> = {
      work: card.is_reversed
        ...
        : `İş bağlamında “${theme}” vurgusu, nerede yeniden hareket alanı açılabileceğine bakmanızı öneriyor. Daha fazla yük almak yerine bu temanın etkilediği tek bir görev ya da iş birliğine odaklanmak daha açıklayıcı olabilir.`,
