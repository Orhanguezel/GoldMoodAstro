...
apt-get install -y -qq \
  curl wget git ufw build-essential ca-certificates rsync unzip dnsutils \
  nginx \
  mysql-server \
  certbot python3-certbot-nginx
# Node.js 22 (NodeSource — Next.js 16 için >=20.9 gerekli, Ubuntu 24.04 default 18)
if ! command -v node >/dev/null || [[ "$(node -v 2>/dev/null | cut -dv -f2 | cut -d. -f1)" -lt 20 ]]; then
  curl -fsSL https://deb.nodesource.com/setup_22.x | bash - >/dev/null
  apt-get install -y -qq nodejs
fi
# Brotli (opsiyonel — Ubuntu 24.04'te ayrı paketler)
apt-get install -y -qq libnginx-mod-http-brotli-filter libnginx-mod-http-brotli-static 2>/dev/null \
  && BROTLI_AVAILABLE=1 || BROTLI_AVAILABLE=0
echo "  → brotli modülü: $([[ $BROTLI_AVAILABLE -eq 1 ]] && echo yes || echo no)"
echo "  → $(nginx -v 2>&1)"
...
# enable yalnızca inactive ise (mevcut SSH session drop'u önle)
...
remote_sh <<BASH
set -euo pipefail
# İlk install'da root → unix_socket auth, sonra password auth.
# Hangisi çalışıyorsa onu kullan.
if mysql --protocol=socket -uroot -e "SELECT 1" >/dev/null 2>&1; then
  MYSQL_AUTH=(--protocol=socket -uroot)
elif mysql -uroot -p'${PROD_DB_ROOT_PASSWORD}' -e "SELECT 1" >/dev/null 2>&1; then
  MYSQL_AUTH=(-uroot -p'${PROD_DB_ROOT_PASSWORD}')
else
  echo "❌ MySQL root erişimi yok (ne socket ne password çalışıyor)"; exit 1
fi
...
# ─── Feature flags ───
FEATURE_VIDEO_CALL=${FEATURE_VIDEO_CALL}
FEATURE_BIRTH_CHART=${FEATURE_BIRTH_CHART}
FEATURE_DAILY_READING=${FEATURE_DAILY_READING}
...
# Mozilla intermediate, generated by GoldMoodAstro provisioning
