// =============================================================
// FILE: src/integrations/rtk/tags.ts
// RTK Query cache/tag listesi (goldmoodastro modülleri)
// =============================================================

export const metahubTags = [
  'Auth',
  'User',
  'AdminUsers',
  'Profiles',
  'Profile',
  'UserRoles',
  'UserRole',
  'SiteSettings',
  'SiteSettingsBulk',
  'CustomPages',
  'CustomPageSlug',
  'CustomPage',
  'Faqs',
  'Services',
  'MenuItems',
  'MenuItem',
  'MenuItemPublic',
  'Slider',
  'Contacts',
  'EmailTemplates',
  'FooterSections',
  'Mail',
  'Newsletter',
  'Notifications',
  'Reviews',
  'Support',
  'Storage',
  'DbAdmin',
  'Sliders',
  'EmailTemplate',
  'Notification',
  'Review',
  'Health',
  'FooterSectionsBySlug',
  'AuditAuthEvent',
  'AuditRequestLog',
  'AuditMetric',
  'DbSnapshot',
  'DbAdmin',
  'DbModule',
  'DbManifest',
  'Bookings',
  'Booking',
  'BookingSlotAvailability',
  'BookingSlots',
  'BookingWorkingHours',
  'BookingWorkingHour',
  'BookingResources',
  'BookingResource',
  'AvailabilityWH',
  'AvailabilityRecurring',
  'AvailabilitySlots',
  'AvailabilityPlan',
  'AvailabilityCheck',
  'Resources',
  'ResourceWorkingHours',
  'ResourceWorkingHour',
  'WorkingHours',
  'WorkingHour',
  'WeeklyPlan',
  'ChatThreads',
  'ChatMessages',
  'ChatKnowledge',
  'Consultants',
  'Consultant',
  'ConsultantSlots',
  'ConsultantAvailability',
  'Favorites',
  'MediaMessages',
  'MediaSettings',
  'BirthCharts',
  'ReviewOutcome',
  'ConsultantOutcomeScore',
  'History',
  'Credits',
  'History',
  'Horoscope',
  'SignInfo',
  'Synastry',
  'ConsultantSelfReviews',
  'ConsultantSelf',
  'ConsultantSelfAvailability',
  'ConsultantSelfBlogPosts',
  'ConsultantSelfBookings',
  'ConsultantSelfClients',
  'ConsultantSelfCompletion',
  'ConsultantSelfProfileViews',
  'ConsultantSelfServiceTemplates',
  'ConsultantSelfServices',
  'ConsultantSelfStats',
  'ConsultantSelfThread',
  'ConsultantSelfThreads',
  'ConsultantSelfWallet',
  'CustomerThread',
  'CustomerThreads',
  'ConsultantApplication',
  'ConsultantApplications',
  'Languages',
  'Commission',
  'ConsultantWithdrawals',
] as const;

export type MetahubTag = (typeof metahubTags)[number];
