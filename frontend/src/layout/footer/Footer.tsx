...
export type FooterProps = {
  ...
  /** Verified social profiles fetched on SSR for crawlable footer links. */
...
const Footer: React.FC<FooterProps> = ({
  ...
  const hasInitialSocials = Boolean(initialSocials && Object.values(initialSocials).some(Boolean));
  const { data: socialsSetting } = useGetSiteSettingByKeyQuery(
    { key: 'socials', locale },
    { skip: hasInitialSocials },
  );

  const socials = useMemo(() => {
    if (hasInitialSocials) return { ...initialSocials };
    const socialsVal = (socialsSetting?.value ?? {}) as Record<string, string>;
    return { ...socialsVal };
  }, [hasInitialSocials, initialSocials, socialsSetting?.value]);
