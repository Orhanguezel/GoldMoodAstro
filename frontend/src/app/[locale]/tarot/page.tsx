...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    pathname: '/tarot',
    fallback: {
      title: `Ücretsiz Tarot Falı ve Kart Anlamları — ${brand.name}`,
  ...
  const intro =
    locale === 'tr'
      ? {
          ...
          summary:
            `Tarot kesin gelecek söylemez; mevcut enerji, soru ve seçenekler üzerine düşünmek için sembolik bir alan açar. ${brand.name}’da tarot; tek kart, üç kart ve daha derin açılımlarla karar, ilişki, kariyer ve içsel farkındalık konularında sorumlu bir rehberlik diliyle yorumlanır.`,
          sections: [
            {
            ...
            {
              title: `${brand.name} tarot yaklaşımı`,
              paragraphs: [
  ...
  return (
    <PageContainer className="min-h-screen bg-(--gm-bg)" verticalPadding="large">
      ...
      <AuthorBio
        ...
        bio={locale === 'tr'
          ? `${brand.name} tarot içerikleri, kart sembollerini gündelik farkındalık ve sorumlu ruhsal rehberlik diliyle yorumlamak için hazırlanır.`
