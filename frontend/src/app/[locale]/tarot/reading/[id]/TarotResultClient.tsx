  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 space-y-8">
        ...
        <p className="text-muted-foreground font-serif italic tracking-widest animate-pulse">{ui('ui_results_tarot_loading', 'Cosmic secrets are filtering through...')}</p>
      </div>
    );
  }
  ...
  if (error || !res) {
    return (
      <div className="text-center py-40 space-y-8">
        ...
        <p className="text-muted-foreground">{ui('ui_results_tarot_not_found_desc', 'The tarot record you are looking for does not exist or an error occurred.')}</p>
        <button
          onClick={() => router.push(`/${locale}/tarot`)}
          className="inline-flex items-center gap-2 text-brand-gold font-bold"
        >
          {ui('ui_results_tarot_new_spread_long', 'START NEW SPREAD')}
        </button>
      </div>
    );
  }
    ...
    <div className="max-w-[var(--gm-w-wide)] mx-auto px-4 py-12 md:py-20 min-h-[80vh] flex flex-col">
      ...
      >
        <div className="text-center">
          <h1 className={`${cinzel.className} text-4xl md:text-6xl text-foreground mb-6`}>{ui('ui_results_tarot_title', 'Cosmic Answer')}</h1>
          <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-brand-primary/10 border border-brand-gold/20">
            ...
            <span className="text-muted-foreground italic font-serif">&quot;{drawResult.question || ui('ui_results_tarot_general_guidance', 'General Guidance')}&quot;</span>
          </div>
        </div>
            ...
            >
              ...
              {card.is_reversed && (
                <div className="flex items-center gap-2 text-xs font-bold text-[var(--gm-error)] uppercase tracking-widest mb-4">
                  <RotateCcw className="w-3 h-3" /> {ui('ui_results_tarot_reversed', 'REVERSED')}
                </div>
              )}
            </motion.div>
          ))}
        ...
        >
          ...
          <div className="flex items-center gap-4 mb-10">
            ...
            <h2 className={`${cinzel.className} text-3xl text-foreground`}>{ui('ui_results_tarot_your_reading', 'Your Reading')}</h2>
          </div>
          ...
          <div className="mt-16 pt-10 border-t border-border/20 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="text-sm text-muted-foreground italic font-serif">
              {ui('ui_results_tarot_disclaimer', '* This interpretation was created from astrologer-approved card symbols and meanings.')}
            </div>
            <div className="flex items-center gap-4">
              <ShareCard
                title={ui('ui_results_tarot_share_title', 'Share My Tarot Reading')}
                shareText={`I had a tarot reading on ${brand.name}\nMy cards: ${drawResult.cards.map((c: any) => c.name).join(', ')}\nBring light to your future too:`}
                variant="tarot"
                data={{
              ...
              >
                {ui('ui_results_tarot_new_spread', 'NEW SPREAD')} <RotateCcw className="w-4 h-4" />
              </button>
            </div>
          </div>
        </motion.div>
