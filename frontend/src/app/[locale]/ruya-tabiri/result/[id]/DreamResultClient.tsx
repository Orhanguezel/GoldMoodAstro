  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 space-y-8">
        ...
        <p className="text-muted-foreground font-serif italic tracking-widest animate-pulse">{ui('ui_results_dream_loading', 'Decoding the dream secrets...')}</p>
      </div>
    );
  }
  ...
  if (error || !res) {
    return (
      <div className="text-center py-40 space-y-8">
        ...
        <p className="text-muted-foreground">{ui('ui_results_dream_not_found_desc', 'The dream record you are looking for does not exist or an error occurred.')}</p>
        <button
          onClick={() => router.push(`/${locale}/ruya-tabiri`)}
          className="inline-flex items-center gap-2 text-brand-primary font-bold"
        >
          {ui('ui_results_dream_new', 'INTERPRET NEW DREAM')}
        </button>
      </div>
    );
  }
    ...
    <div className="max-w-[var(--gm-w-content)] mx-auto px-4 py-12 md:py-20 min-h-[80vh] flex flex-col">
      ...
      >
        <div className="text-center space-y-4">
          <span className="text-xs font-bold tracking-[0.3em] text-brand-primary uppercase opacity-60">{ui('ui_results_dream_subconscious', 'Subconscious Reflection of the Dream')}</span>
          <h1 className={`${cinzel.className} text-5xl md:text-7xl text-foreground`}>{ui('ui_results_dream_title', 'Dream Analysis')}</h1>
        </div>
          ...
          {result.symbols?.map((s: any, i: number) => (
            ...
            >
              ...
              <div className="text-[10px] font-bold text-brand-primary/60 uppercase tracking-widest mb-3">
	                %{Math.round((s.confidence || 0.9) * 100)} {ui('ui_results_dream_confidence', 'Confidence')}
              </div>
              {s.meaning && (
          ...
          <div className="mt-20 pt-10 border-t border-[var(--gm-text)]/5 flex flex-col lg:flex-row items-center justify-between gap-10">
            <div className="flex flex-col gap-2 text-center lg:text-left">
              <p className="text-sm text-muted-foreground italic font-serif opacity-50">
	                {ui('ui_results_dream_disclaimer', '* This interpretation is generated by AI and blended with Jungian archetypes and ancient symbolism.')}
              </p>
            </div>
            
            <div className="flex items-center gap-4">
              <ShareCard
	                title={ui('ui_results_dream_share_title', 'Share My Dream Reading')}
	                shareText={`I had my dream interpreted on GoldMoodAstro\nMy symbols: ${result.symbols.map((s: any) => s.name).join(', ')}\nDiscover the hidden message of the subconscious:`}
                variant="dream"
                data={{
              ...
              >
	                {ui('ui_results_dream_new', 'INTERPRET NEW DREAM')} <RotateCcw className="w-4 h-4" />
              </button>
            </div>
          </div>
        </div>
