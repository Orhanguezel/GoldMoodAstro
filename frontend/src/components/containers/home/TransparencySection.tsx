    ...
    <section className="py-32 bg-(--gm-bg) border-t border-(--gm-border-soft)">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-16 reveal">
          <span className="section-label">{ui("ui_home_transparency_label", "Membership")}</span>
          <h2 className="font-serif text-[clamp(2.5rem,5vw,4rem)] font-light leading-tight text-(--gm-text)">
            {ui("ui_home_transparency_title_1", "Transparent pricing,")}<br/>
            <em className="text-(--gm-gold) italic">{ui("ui_home_transparency_title_2", "no hidden terms.")}</em>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 reveal items-stretch">
          ...
          <div className="bg-(--gm-surface) border border-(--gm-border-soft) rounded-2xl p-10 relative transition-all duration-500 hover:-translate-y-2 hover:shadow-(--gm-shadow-card) hover:border-(--gm-gold)/40 group flex flex-col">
            <div className="absolute top-6 right-6 font-display text-[9px] tracking-[0.3em] uppercase py-1.5 px-3 border border-(--gm-gold)/30 text-(--gm-gold-dim) rounded-full">
              {ui("ui_home_transparency_free_badge", "Free")}
            </div>
            ...
            <ul className="space-y-4 mb-10 text-sm text-(--gm-text-dim) flex-grow">
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/50 mt-1">✦</span> {ui("ui_home_transparency_free_feature_1", "Basic birth chart view")}
              </li>
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/50 mt-1">✦</span> {ui("ui_home_transparency_free_feature_2", "Short daily reading")}
              </li>
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/50 mt-1">✦</span> {ui("ui_home_transparency_free_feature_3", "Sign compatibility (summary)")}
              </li>
            </ul>
            <Link href={`/${locale}/register`} className="w-full py-4 text-center rounded-full border border-(--gm-border) text-(--gm-text) text-xs font-bold uppercase tracking-[0.2em] hover:bg-(--gm-surface-high) transition-all mt-auto">
              {ui("ui_home_transparency_free_cta", "Start for Free")}
            </Link>
          </div>
          ...
          >
            <div className="absolute top-6 right-6 font-display text-[9px] tracking-[0.3em] uppercase py-1.5 px-3 border border-(--gm-gold) text-(--gm-bg-deep) font-bold bg-(--gm-gold) rounded-full shadow-[0_0_15px_rgba(212,175,55,0.4)]">
              {ui("ui_home_transparency_recommended_badge", "Recommended")}
            </div>
            ...
            <ul className="space-y-4 mb-10 text-sm text-(--gm-text) flex-grow">
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold) mt-1 drop-shadow-[0_0_5px_rgba(212,175,55,0.5)]">✦</span> {ui("ui_home_transparency_monthly_feature_1", "Detailed birth chart analysis")}
              </li>
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold) mt-1 drop-shadow-[0_0_5px_rgba(212,175,55,0.5)]">✦</span> {ui("ui_home_transparency_monthly_feature_2", "Daily · weekly · monthly forecast")}
              </li>
              <li className="flex items-start gap-3">
          ...
          <div className="bg-(--gm-surface) border border-(--gm-border-soft) rounded-2xl p-10 relative transition-all duration-500 hover:-translate-y-2 hover:shadow-(--gm-shadow-card) hover:border-(--gm-gold)/40 group flex flex-col">
            <div className="absolute top-6 right-6 font-display text-[9px] tracking-[0.3em] uppercase py-1.5 px-3 border border-(--gm-gold)/30 text-(--gm-gold) bg-(--gm-gold)/10 rounded-full">
              {ui("ui_home_transparency_best_value_badge", "Best Value")}
            </div>
            <div className="font-display text-[14px] tracking-[0.32em] text-(--gm-gold-dim) uppercase mb-6">
            ...
            <ul className="space-y-4 mb-10 text-sm text-(--gm-text-dim) flex-grow">
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/80 mt-1">✦</span> {ui("ui_home_transparency_yearly_feature_1", "All features in Monthly")}
              </li>
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/80 mt-1">✦</span> {ui("ui_home_transparency_yearly_feature_2", "20% discount for astrologer sessions")}
              </li>
              <li className="flex items-start gap-3">
                <span className="text-(--gm-gold)/80 mt-1">✦</span> {ui("ui_home_transparency_yearly_feature_3", "VIP priority support")}
              </li>
              <li className="flex items-start gap-3">
        ...
        <div className="mt-20 flex flex-col md:flex-row items-center gap-6 p-8 border border-(--gm-gold)/20 bg-(--gm-gold)/5 rounded-2xl reveal">
          ...
          <div>
            <div className="font-display text-[11px] tracking-[0.2em] text-(--gm-gold) uppercase mb-2 font-bold">
              {ui("ui_home_transparency_guarantee_title", "Transparency Guarantee")}
            </div>
            <p className="text-(--gm-text-dim) text-sm font-light leading-relaxed">
              {ui("ui_home_transparency_guarantee_desc", "Free trial starts without asking for a credit card. You manually approve the transition to Premium — no one charges you without your knowledge. Canceling is just a click away.")}
            </p>
          </div>
        </div>
      </div>
