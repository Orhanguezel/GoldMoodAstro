...
type ContentModuleClientProps = {
  ...
  allowCreate?: boolean;
  aiCaution?: string;
  previewPath?: (post: CustomPageDto, locale: string) => string | null;
};

function defaultPreviewPath(post: CustomPageDto, locale: string): string | null {
  const loc = locale || 'tr';
  if (post.module_key === 'blog' && post.slug) return `/${loc}/blog/${post.slug}`;
  if (post.module_key === 'landing' && post.landing_key) return `/${loc}/${post.landing_key}`;
  if (post.slug) return `/${loc}/${post.slug}`;
  return null;
}

export default function ContentModuleClient({
  moduleKeys,
  title,
  description,
  eyebrow,
...
}: ContentModuleClientProps) {
  const primaryModuleKey = moduleKeys[0] || 'page';
  const moduleKeySet = React.useMemo(() => new Set(moduleKeys), [moduleKeys]);
  ...
  const query = useListCustomPagesAdminQuery({
    module_key: moduleKeys.length === 1 ? primaryModuleKey : undefined,
    locale,
    q: q.trim() || undefined,
    sort: 'created_at',
    orderDir: 'desc',
  ...
  const runAi = async (action: AiAction) => {
    ...
    try {
      const res = await aiAssist({
        ...
        locale: form.locale,
        title: form.title,
        category: [form.tags, aiCaution].filter(Boolean).join('\n'),
  ...
  const save = async () => {
    ...
    const payload = {
      module_key: form.module_key || primaryModuleKey,
      landing_key: form.landing_key || null,
      locale: form.locale,
      title,
      slug,
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gm-gold">{eyebrow || b('eyebrow', 'İçerik Yönetimi')}</span>
          </div>
        ...
        <Card className="rounded-[28px] border-gm-border-soft bg-gm-surface/30 shadow-xl">
          <CardContent className="space-y-5 p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="font-serif text-2xl text-gm-text">{editing ? formEditTitle || b('form.editTitle', 'Yazıyı Düzenle') : formNewTitle || b('form.newTitle', 'Yeni Blog Yazısı')}</h2>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <Field label={b('form.fields.cover', 'Kapak Görseli')}>
                    <AdminImageUploadField
                      ...
                      bucket="public"
                      folder={storageFolder || primaryModuleKey}
                      previewAspect="16x9"
                    />
                  </Field>
