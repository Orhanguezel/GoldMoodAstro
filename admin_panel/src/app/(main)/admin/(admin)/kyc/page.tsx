...
export default function KycAdminPage() {
  ...
  const handleApprove = async (id: string) => {
    if (!confirm('Bu danışmanın KYC belgelerini onaylamak istediğinize emin misiniz?')) return;
    try {
      await approve(id).unwrap();
      toast.success('KYC başarıyla onaylandı.');
  ...
  const handleReject = async (id: string) => {
    if (!rejectReason) {
      toast.error('Lütfen bir ret sebebi giriniz.');
      return;
    }
    if (!confirm('KYC başvurusunu reddetmek istediğinize emin misiniz?')) return;
    try {
      ...
      toast.error('Ret işlemi sırasında bir hata oluştu.');
    }
  };

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">KYC Onay Bekleyen Danışmanlar</h1>
      
      {isLoading ? (
        <div>Yükleniyor...</div>
      ) : pendingKyc.length === 0 ? (
        <div className="text-gray-500">Bekleyen KYC başvurusu bulunmuyor.</div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {pendingKyc.map(consultant => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold">{consultant.full_name || 'İsimsiz'}</h3>
                    ...
                    <div className="mt-2 text-xs font-semibold px-2 py-1 bg-gray-100 rounded-full inline-block">
                      {consultant.account_type === 'company' ? 'Şirket' : 'Bireysel'}
                    </div>
                  </div>
          ...
          {selectedConsultant && (
            <div className="p-6 border rounded-xl bg-white sticky top-6">
              <h2 className="text-xl font-bold mb-4">Detaylar</h2>
              
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="block text-gray-500">Hesap Tipi</span>
                    <span className="font-medium">{selectedConsultant.account_type === 'company' ? 'Şirket' : 'Bireysel'}</span>
                  </div>
                  <div>
                    <span className="block text-gray-500">Fatura Adresi</span>
                  ...
                  {selectedConsultant.account_type === 'individual' ? (
                    <>
                      <div>
                        <span className="block text-gray-500">TC Kimlik No</span>
                    ...
                    <>
                      <div>
                        <span className="block text-gray-500">Şirket Unvanı</span>
                        <span className="font-medium">{selectedConsultant.company_name || '-'}</span>
                      </div>
                      <div>
                        <span className="block text-gray-500">Vergi Dairesi</span>
                        <span className="font-medium">{selectedConsultant.tax_office || '-'}</span>
                      </div>
                      <div>
                        <span className="block text-gray-500">Vergi No</span>
                ...
                <div className="pt-4 border-t">
                  <h3 className="font-bold mb-2">Yüklenen Belgeler</h3>
                  {selectedConsultant.kyc_documents && selectedConsultant.kyc_documents.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500">Belge yüklenmemiş.</div>
                  )}
                </div>
                ...
                <div className="pt-6 border-t flex flex-col gap-3">
                  ...
                  <div className="mt-4 p-4 border border-red-100 rounded-lg bg-red-50">
                    <h4 className="font-bold text-red-800 text-sm mb-2">Reddet</h4>
                    <textarea 
                      value={rejectReason}
                      onChange={e => setRejectReason(e.target.value)}
                      placeholder="Reddetme sebebini yazın (Danışmana email ile gidecek)"
                    ...
                    >
                      {isRejecting ? 'Reddediliyor...' : 'Reddet ve Bildir'}
                    </button>
                  </div>
                </div>
              </div>
