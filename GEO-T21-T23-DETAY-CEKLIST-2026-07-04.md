...
üretiyor: 12 burç × 3 dil, aynı ~10 uzun cümle kalıbına yalnızca `s.label_tr, s.element_tr,
s.core_tr, s.love_tr, s.shadow_tr, s.career_tr, s.wellness_tr` değişkenleri enjekte ediliyor.
...
- [ ] **T21-0 — Duplikasyon ölçüm scripti (önce ölç, sonra düzelt).**
...
- [ ] **T21-1 — İçerik üretim briefi + hammadde konsolidasyonu.**
...
- [ ] **T21-2 — TR içerik üretimi (LLM destekli + kural denetimli).**
...
- [ ] **T21-3 — EDİTÖR ONAYI (blocker gate — insan).**
  12 markdown dosyası Pınar Demircioğlu / site sahibine onaya gider. Onay işareti dosya başına
...
- [ ] **T21-5 — Alt sayfa farklılaştırması (kariyer/saglik/ask).**
...
- [ ] **T23-0 — Bundle analizi (önce ölç).** `@next/bundle-analyzer` devDep ekle;
...
- [ ] **T23-1 — uiDb sözlüğünü client bundle'dan çıkar (ana kazanım).**
...
- [ ] **T23-3 — Ana sayfa section'larını code-split et.**
...
- [ ] **T23-4 — framer-motion + swiper diyeti.**
...
- [ ] **T23-5 — LCP görseli son ayar.** Hero görseline responsive `sizes` + mobil için küçük
...
- [ ] **T23-6 — Ölçüm protokolü (429 çözümü) + kapanış ölçümü.**
  ...
  (`bun run psi:mobile`). Alternatif lokal doğrulama: `bunx lighthouse https://goldmoodastro.com/tr
  --preset=perf --emulated-form-factor=mobile --throttling-method=simulate --output=json`.
