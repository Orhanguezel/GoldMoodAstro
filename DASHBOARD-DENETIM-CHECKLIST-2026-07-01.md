...
### 🟡 Orta / İyileştirme
- [ ] "Member since" görünmüyor — `/auth/user` `created_at` dönmüyor (`dashboard/page.tsx:278-283`).
- [ ] `UserMessagesPanel` `isTr` ölü + tarih hep `tr-TR` (21,35-39); `BookingMessageButton.tsx:37` aynı.
- [ ] Sekme geçişi `replaceState` vs kart `Link` push — history tutarsız.
- [ ] Login redirect `?tab` kaybediyor (`dashboard/page.tsx:195`).
...
- [ ] me/credits balance hatası "0" olarak gizleniyor (`isError` göster).
- [ ] Review modal `targetId` fallback şüpheli (`consultant_id || resource_id`) — `consultant_id` yoksa butonu gizle.
...
- [ ] **`rejectBooking`'de statü guard yok** — `controller.ts:716-783`. confirmed/completed reddedilebilir, iade/kazanç geri alma yok. **Fix:** `cancellable` liste kontrolü + ödenmişse refund.
...
- [ ] **Optimistic update yalnız filtresiz cache'i yamalıyor** — `consultant_self.endpoints.ts:406-462`. Statü filtresi aktifken approve/reject anında yansımaz.
- [ ] **Cüzdan filtre/CSV yalnız ilk 50 işlemde** — `WalletPanel.tsx:107-123`. `from/to/page/type` FE'den gönderilmiyor → eksik export. **Fix:** query param'ları geçir; CSV için tüm sayfalar.

### 🟡 Orta / İyileştirme
- [ ] Overview acil rozet expired talepler için de yanıyor (backend 5dk filtresi yok) — `controller.ts:899-902`.
- [ ] Paid servis `price 0` çelişkisi (FE≥0 / backend>0) → ham hata kodu (`ServicesPanel.tsx:125,510`).
- [ ] "Loyal" rozeti yanlış (`idx===0`=en son randevu) + iptal/red booking'ler sayılıyor (`ClientsPanel.tsx:128-133`).
- [ ] `ClientsPanel` debounce yok + çifte filtre (44-53).
- [ ] Availability preset tüm haftayı REPLACE ediyor (onay yok) (`AvailabilityPanel.tsx:105-108`).
- [ ] Kapasite kalıntısı: backend `capacity max(100)` hâlâ kabul; eski `capacity>1` geri yazılıyor. **Fix:** backend'de 1'e sabitle.
...
- [ ] `CompletionScoreWidget` ring rengi çalışmıyor (`bg-*` yerine `text-*` olmalı) (`ConsultantDashboard.tsx:1466-1472`).
- [ ] `ConsultantCardPreview` kök div `relative` değil → "Preview" overlay tüm sidebar'ı kaplar (40,117). Ayrıca `₺` hardcoded, "Verified" rozeti approval'dan bağımsız hep görünür, `rating_avg` null → "NaN", video seans fiyatı önizlenmiyor.
- [ ] `MessagesPanel`: thread değişince taslak temizlenmiyor (yanlış kişiye gönderme riski) (41,109); `isError` yutuluyor; okundu çift-işaretleme (GET yan etkisi + `markAsRead`); `tr-TR` hardcoded.
...
- [ ] "Deactivate" hızlı butonu kaydedilmemiş taslağı da persist ediyor (`ServicesPanel.tsx:587`).
- [ ] Boost rozeti metni bozuk birleşim (`{days}` çift) (`ServicesPanel.tsx:553`).
...
- [ ] Cüzdan seçimi `ORDER BY`'sız belirsiz + zorla `consultant_id` bağlama (`controller.ts:1493-1525`).
