...
- [x] SSR/CSR hydration race kontrol (splash flicker) — token ilk boyada doğru
  ...
  - ✅ 2026-06-30 Codex: `gen:theme-snapshot` / `check:theme` kök script'leri doğrulandı; frontend deploy workflow'una `Check theme snapshot drift` adımı eklendi, seed `design_tokens` ↔ `frontend/src/lib/tokens/defaults.ts` drift'i CI'da fail eder.
...
- [x] ✅ **§v2-SCOPE-EXT (Claude 2026-05-16 — "EN çalışmıyor" kök neden):** frontend ~92/144 bileşende gömülü Türkçe → `ui_*` anahtarına çıkar (`useUi`/`uiDb`); `ui_*` seed'e TR+EN; `scan:hardcoded-tr` envanter + `check:i18n` guard (JSX/toast TR literal = fail). **§v2-FINAL tek başına EN'i DÜZELTMEZ** — bu olmadan web'de İngilizce kırık. Öncelik: booking/consultant/auth/ödeme/home. Sahip: Codex/Cursor (frontend), FAZ33 ile tek `ui_*` sözlüğü. Kontrat: [`i18n-single-source-contract.md`](./contracts/i18n-single-source-contract.md) §v2-SCOPE-EXT
  ...
  - [x] 2026-06-29 Codex FAZ 1: web `/de` + `/en` yüksek ROI seed tamamlandı. `016d_ui_extra_seed.sql` 653 `ui_*` anahtarı TR/EN/DE ekler (518 missing JSON + readUi/copyFallback kaynakları + dinamik ay ailesi); `dashboard/page.tsx` history etiketleri `de` kullanır; `016/016a/016b/016c` DE umlaut taraması düzeltildi; ölü `frontend/src/types/locales.json` silindi. Ana §v2-SCOPE-EXT hâlâ açık: Faz 2-4 ve kalan hardcoded kapsam ayrı.
  - [x] 2026-06-29 Codex FAZ 2: danışman bio/headline i18n tablosu + TR/EN/DE seed eklendi; public consultant list/detail/slots locale-aware `bio` döndürür (locale → TR → eski kolon fallback); web consultant SSR/RTK çağrıları locale header taşır. İlk 10 operasyonel e-posta template'i TR/EN/DE tamamlandı; `order_received`, `deposit_success`, `ticket_replied` eksik template key'leri seed'e eklendi.
  - [x] 2026-06-29 Codex FAZ 3 çekirdek: admin blog yönetimi ve consultant applications ekranı `admin.*` JSON i18n'e taşındı (TR/EN/DE); mobil `MobileI18nLocale` + fallback snapshot `de` destekli oldu; `initI18n()` cihaz `de` dilini ve kaydedilmiş dili açılışta tanır; `check:i18n` guard TR/EN/DE drift kontrol eder. Düşük öncelikli mobil hardcoded süpürmesi (`BookingCard`, `ErrorBoundary`, vb.) açık kalır.
  - [x] 2026-06-29 Codex FAZ 4 çekirdek: LLM prompt seed'leri EN/DE tamamlandı (`0993_llm_prompts_i18n_seed.sql`: natal, horoscope, compatibility, tarot, coffee, dream, numerology, yildizname, synastry); horoscope/birth chart/synastry/yildizname LLM değişkenlerinde burç adları locale-aware oldu. Statik KB/katalog içeriklerinin tam TR/EN/DE çevirisi ayrı kalan iş.
  - [x] 2026-06-29 Codex FAZ 4 ticari i18n: banner, campaign, subscription plan ve credit package için yan i18n tabloları + TR/EN/DE seed eklendi; public API yanıtları `locale` ile resolved `title/name/description/cta_label` döndürür, eski `*_tr/*_en` alanları geriye uyumlu kalır.
  - [x] 2026-06-30 Codex FAZ 4 katalog başlangıç: tarot/kahve/rüya statik kataloglarına yan i18n tabloları eklendi; tarot çekilişi, kahve KB ve rüya KB runtime'da locale-aware okur. Başlangıç seed: Major Arcana 22 kart EN/DE, 15 kahve sembolü EN/DE, 15 rüya sembolü EN/DE. Kalan: tüm minor arcana + tüm sembol kataloğu kürasyon çevirisi ve API hata mesajları.
  - [x] 2026-06-30 Codex FAZ 4 tarot tam katalog: `181b_tarot_minor_i18n_seed.sql` ile Minor Arcana 56 kart EN/DE tamamlandı; tarot_card_i18n sayımı EN=78, DE=78, eksik kart=0. Kalan katalog işi kahve/rüya sembollerinin tamamını kürasyon kalitesinde genişletmek.
  - [x] 2026-06-30 Codex FAZ 4 kahve tam katalog: `185b_coffee_symbols_i18n_seed.sql` ile kalan 35 kahve sembolü EN/DE tamamlandı; coffee_symbol_i18n sayımı EN=50, DE=50, eksik sembol=0. Kalan katalog işi rüya sembollerinin tamamını kürasyon kalitesinde genişletmek.
  - [x] 2026-06-30 Codex FAZ 4 rüya tam katalog: `185c_dream_symbols_i18n_seed.sql` ile kalan 85 rüya sembolü EN/DE tamamlandı; dream_symbol_i18n sayımı EN=101, DE=101, eksik sembol=0. Tarot/kahve/rüya statik katalog i18n kapsamı kapandı.
  - [x] 2026-06-30 Codex FAZ 4 API hata mesajı başlangıç sweep: tarot, kahve, rüya, numeroloji, kredi/ödeme, yıldızname, sinastri ve horoskop public hata dönüşleri `apiMessage(req, key)` helper'ına bağlandı (TR/EN/DE; query/header/request locale fallback). Kalan: history/support/auth deletion/live session gibi düşük öncelikli modüllerde aynı sweep.
  - [x] 2026-06-30 Codex FAZ 4 API hata mesajı kalan sweep: history, support public/admin ve auth deletion hata dönüşleri `apiMessage(req, key)` helper'ına bağlandı; response şekilleri (`error` vs `message`) korunarak TR/EN/DE desteklendi.
  - [x] 2026-06-30 Codex FAZ 4 live session ürün metni: görüşme bitişinde kredi bakiyesi yetersiz notification/push başlık ve gövdesi booking locale'e göre TR/EN/DE çözülecek şekilde güncellendi; kredi transaction açıklaması kayıt/denetim açıklaması olarak bırakıldı.
  - 🟡 2026-06-30 Codex §v2-SCOPE-EXT web P1 sweep: `AuthModal`, booking/consultant mesajlaşma butonları-modalı, consultant kart/preview, become-consultant hero/banner, `ProfileViewsPanel`, call page, `ServicesPanel`, `ClientsPanel`, `MessagesPanel`, `WalletPanel`, `AvailabilityPanel` ve `ConsultantDashboard` fallback'leri İngilizce hardFallback + DB `ui_*` kaynaklarına hizalandı; `016/016a/016b/016d` ilgili anahtarları eklendi/güncellendi. `i18n:scan` P1 404 → 175, toplam yüksek bulgu 1788 → 1568; guard hâlâ pre-existing 32 dosya artışı nedeniyle fail.
  - 🟡 2026-06-30 Codex devam: web P1 kullanıcı yolu tamamlandı. `BecomeConsultantPage`, `UserMessagesPanel`, `dashboard/page.tsx`, `ConsultantDetail`, `ReviewsPanel`, auth login/register, booking/payment callback, consultant browse/list/filters/slot, social login ve ilgili route metadata/dev banner fallback'leri İngilizce fallback + DB `ui_*` kaynaklarına hizalandı; `016_ui_auth_seed.sql`, `016d_ui_extra_seed.sql`, `frontend/src/i18n/uiDb.ts` yeni/güncel anahtarlarla doğrulandı. `i18n:scan` P1 404 → 0, toplam yüksek bulgu 1788 → 1305; guard hâlâ P2/P3 kapsamındaki 29 pre-existing artış dosyası nedeniyle fail.
  - ✅ 2026-06-30 Codex devam: P2/P3 guard artışı kapatıldı. Expertise/home promo/avatar/cookie/not-found/header/editor, zodiac/daily/birth-chart/tarot/yildizname/readings/privacy fallback'leri İngilizce hardFallback + mevcut/yeni `ui_*` anahtarlarına hizalandı; `016_ui_copy_seed.sql`, `016d_ui_extra_seed.sql`, `frontend/src/i18n/uiDb.ts` güncellendi. Doğrulama: `frontend bun run typecheck` ✅, `frontend bun run i18n:scan` ✅ (`P1=0`, toplam yüksek 970, düşük 6), `backend bun src/db/index.ts --no-drop --only=016` ✅.
  - 🟡 2026-06-30 Codex kalan temizlik: pricing/sinastri/tarot/numeroloji sayfaları, zodiac affirmation/celebrity veri setleri, burç detay/uyum/alt sayfa metadata fallback'leri, home daily/zodiac grid fallback'leri, yükselen/burç finder sayfaları ve zodiac OG görselleri İngilizce/ASCII kaynakla temizlendi. Doğrulama: `frontend bun run typecheck` ✅, `frontend bun run i18n:scan` ✅ (`P1=0`, `P2=99`, toplam yüksek 620, düşük 6).
  - 🟡 2026-06-30 Codex P2 kapama + P3 sweep: me/settings-credits, zodiac compatibility, tarot/sinastri/kahve/rüya/yıldızname result client/metadata fallback'leri temizlendi; ardından About/Editorial/SupportBot/ShareCard, header, rich editor, contact, coffee, blog list/detail, dream, password reset ve karne P3 fallback'leri İngilizce/ASCII kaynağa çekildi. Doğrulama: `frontend bun run typecheck` ✅, `frontend bun run i18n:scan` ✅ (`P1=0`, `P2=0`, `P3=273`, toplam yüksek 269, düşük 4).
  - ✅ 2026-06-30 Codex final hardcoded-TR kapama: kalan P3 dosyaları temizlendi; home Promises/Hybrid/Trust/Intro ve footer CTA/security metinleri `ui_*` sözlüğü + TR/EN/DE seed'e bağlandı; frontend fallback'leri İngilizce/ASCII hizasına çekildi. Doğrulama: `frontend bun run typecheck` ✅, `frontend bun run i18n:scan` ✅ (`P1=0`, `P2=0`, `P3=0`, toplam yüksek 0, düşük 0).
