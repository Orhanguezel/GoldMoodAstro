...
| 3 | 🟢 **T42-4 §v2-FINAL ONAYLANDI** (kullanıcı: dinamik, TR+EN, hard kod YOK) — mobil `i18n.ts` 513 satır inline **SİL** → backend `siteSettings ui_*` fetch (TR+EN); offline = `gen:i18n-snapshot` ile **otomatik üretilen** snapshot + runtime cache (elle metin YOK); `UI_FALLBACK_EN` de generated'a çevrilir; ölü `i18n/request.ts`+`messages/` cleanup (önce next.config teyit); CI `check:i18n` (parite+stale fail) | [`i18n-single-source-contract.md`](doc/contracts/i18n-single-source-contract.md) **§v2-FINAL** | Codex/Cursor | Cursor mobilde aktif → bu ona; Claude PageContainer'da (çakışmaz) |
