...
| 3b | 🔴 **T42-4 §v2-SCOPE-EXT** (kullanıcı 2026-05-16: "EN çalışmıyor") — **frontend ~92/144 bileşende gömülü Türkçe** → `ui_*` anahtarına çıkar (toast/label/placeholder/JSX text); `useUi`/`uiDb` ile oku; `ui_*` seed'e TR+EN; `scan:hardcoded-tr` envanter + `check:i18n` guard (TR literal fail). Bu OLMADAN EN web'de KIRIK kalır. Öncelik: booking/consultant/auth/ödeme/home | i18n contract **§v2-SCOPE-EXT** | Codex/Cursor (frontend) | büyük, sistematik; FAZ33 ile aynı kök, tek ui_* sözlüğü; Claude scope+guard verdi |
