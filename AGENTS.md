...
| 3a | 🟢 **T42-4 §v2-FINAL** (mobil) — `i18n.ts` 513 satır inline **SİL** → backend `siteSettings ui_*` fetch (TR+EN); offline = `gen:i18n-snapshot` otomatik snapshot + runtime cache; `UI_FALLBACK_EN` generated; ölü `i18n/request.ts`+`messages/` cleanup; CI `check:i18n` | [`i18n-single-source-contract.md`](doc/contracts/i18n-single-source-contract.md) **§v2-FINAL** | Codex/Cursor | Cursor mobilde aktif |
| 3b | 🔴 **T42-4 §v2-SCOPE-EXT** (kullanıcı: "EN çalışmıyor") — envanter ✅ ÇIKTI: **178 dosya / 1723 yüksek bulgu** (P1=409/32 dosya). Gömülü TR → `ui_*` (TR+EN seed, `useUi`/`uiDb`). Araç: `bun run i18n:scan` (rapor) / `i18n:guard` (artış fail, baseline commit'li). **P1 önce:** ConsultantDashboard(86)/BecomeConsultant(40)/ServicesPanel(34)/ConsultantDetail(27)/Availability(22)/Reviews(21)/auth. ⚠️ `checkout/[orderId]` Almanca kalıntı da temizle. Bu olmadan EN KIRIK | i18n contract **§v2-SCOPE-EXT** | Codex/Cursor (frontend) | büyük; FAZ33 ile tek ui_* sözlüğü; Claude envanter+guard verdi |
| 4 | ✅ **T38-1 migrasyon FİİLEN TAMAM** (Claude doğrulama 2026-05-16) — 62/68 page.tsx + delegatör client bileşenleri (DailyPageClient/PricingPageClient/HomeContent) `<PageContainer>` kullanıyor; kalan 6 = redirect/app-shell/full-bleed (by-design, hedef değil). Ek iş YOK | theme-arch §1 | — | kapandı |
| 5 | ✅ **T38-4 runbook DONE** (Cursor) + 🟢 **R3/T38-3 KARAR** (kullanıcı 2026-05-16): tema DİNAMİK, admin `siteSettings design_tokens` seçer — kod marka rengine karar VERMEZ ("gold mu amethyst mi" geçersiz; canlı amethyst = admin seçimi). `defaults.ts`/`UI_FALLBACK_EN` = seed'den `gen:theme-snapshot` ile **otomatik üretilen** ayna (elle renk YOK; i18n #3 ile aynı no-hardcode prensibi). Kalan: Codex `gen:theme-snapshot` + seed↔defaults drift CI + globals.css fallback hizası | theme-arch §3 (güncel) | Codex | i18n #3 ile aynı desen |
