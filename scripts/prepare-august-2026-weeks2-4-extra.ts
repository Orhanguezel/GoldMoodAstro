...
const specs: Spec[] = [
  ...
  { day: 11, slug: "new-moon-rising", title: "Kapısı Açılacak Yükselenler", subtitle: "Yeniay ön hazırlığı", body: "Koç, Aslan ve Yay yükselenler görünürlük; Boğa, Başak ve Oğlak düzen alanında hareket hissedebilir.", cta: "Yükselenini yorumlara yaz", secondTitle: "Yükselenini Bilmiyorsan", secondBody: "Yükselen doğum SAATİ olmadan hesaplanamaz. Nüfus kaydındaki saatle siteden doğum haritanı çıkarabilirsin.", asset: zodiac("aries"), secondAsset: zodiac("leo") },
  { day: 12, slug: "solar-eclipse", title: "Yeniay ve Güneş Tutulması", subtitle: "Yeni bir sayfa açılıyor", body: "Hızlı karar yerine net niyet. Görünür olmak istediğin alanı seç ve küçük bir adım belirle.", cta: "Kaydet • kişisel yorum için siteyi ziyaret et", secondTitle: "Tutulmada Ne Yapılmaz?", secondBody: "Tutulma günü geri dönüşü olmayan büyük kararlar için uygun değil. Niyeti bugün yaz, uygulamayı birkaç güne bırak.", asset: dream("sun"), secondAsset: dream("moon"), campaign: true },
  ...
  { day: 26, slug: "rising-relief", title: "Rahat Nefes Alacak Yükselenler", subtitle: "Dolunay ön hazırlığı", body: "Yengeç, Akrep ve Balık yükselenler duygusal; İkizler, Terazi ve Kova iletişim alanında netleşebilir.", cta: "Kaydet • yükseleninle tekrar bak", secondTitle: "Rahatlama Nasıl Gelir?", secondBody: "Zorlamayla değil bırakmayla. Bu hafta bir sorumluluğu devret ya da verdiğin bir 'evet'i geri al.", asset: zodiac("cancer"), secondAsset: zodiac("scorpio") },
...
async function build(): Promise<DraftPost[]> {
  ...
  for (const s of specs) {
    ...
    if (s.reel) posts.push(await reel(s.day, s.slug, s.title, cover, cap(s)));
      ...
      posts.push(await carousel(s.day, s.slug, s.title, [cover, second], cap(s), s.campaign ? "kampanya" : "etkilesim"));
    }
