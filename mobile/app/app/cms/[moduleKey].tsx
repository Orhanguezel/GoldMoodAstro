import React, { useEffect, useMemo, useState } from 'react';
import { View, Text, StyleSheet, Pressable } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useLocalSearchParams } from 'expo-router';
import { ChevronLeft } from 'lucide-react-native';
import { useAppTheme, type AppTheme } from '@/theme';
import { safeRouterBack } from '@/lib/navigation';
import { customPagesApi } from '@/lib/api';
import { extractHtmlFromPage, pickPublishedPages, type CustomPageRow } from '@/lib/cms';
import { CmsHtmlView } from '@/components/CmsHtmlView';

function buildStyles(t: AppTheme) {
  const { colors, spacing, font } = t;
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: colors.bg },
    header: {
      flexDirection: 'row',
      alignItems: 'center',
      paddingHorizontal: spacing.lg,
      paddingVertical: spacing.md,
      gap: 12,
      borderBottomWidth: StyleSheet.hairlineWidth,
      borderBottomColor: colors.lineSoft,
    },
    backBtn: {
      width: 40,
      height: 40,
      borderRadius: 20,
      backgroundColor: colors.surface,
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: 1,
      borderColor: colors.line,
    },
    title: { flex: 1, fontFamily: font.display, fontSize: 18, color: colors.text },
  });
}

export default function CmsModuleScreen() {
  const theme = useAppTheme();
  const styles = useMemo(() => buildStyles(theme), [theme]);
  const { colors } = theme;

  const params = useLocalSearchParams<{
    moduleKey?: string | string[];
    title?: string | string[];
    locale?: string | string[];
  }>();

  const moduleKey = Array.isArray(params.moduleKey) ? params.moduleKey[0] : params.moduleKey ?? '';
  const fallbackTitle = Array.isArray(params.title) ? params.title[0] : params.title ?? 'Sayfa';
  const locale = Array.isArray(params.locale) ? params.locale[0] : params.locale ?? 'tr';

  const [page, setPage] = useState<CustomPageRow | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!moduleKey) return;
    setLoading(true);
    customPagesApi
      .list({ module_key: moduleKey, locale, limit: 5, orderDir: 'asc' })
      .then((rows) => setPage(pickPublishedPages(rows)[0] ?? null))
      .catch(() => setPage(null))
      .finally(() => setLoading(false));
  }, [moduleKey, locale]);

  const html = extractHtmlFromPage(page);
  const screenTitle = page?.title?.trim() || fallbackTitle;

  return (
    <View style={styles.container}>
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <View style={styles.header}>
          <Pressable style={styles.backBtn} onPress={() => safeRouterBack()}>
            <ChevronLeft size={24} color={colors.text} />
          </Pressable>
          <Text style={styles.title} numberOfLines={2}>
            {screenTitle}
          </Text>
        </View>
        <CmsHtmlView html={html} loading={loading} />
      </SafeAreaView>
    </View>
  );
}
